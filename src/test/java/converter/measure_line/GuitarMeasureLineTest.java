...
public class GuitarMeasureLineTest {
    ...
    void testValidity1(){
        ...
        TabString measureLine = new GuitarMeasureLine(s1, new String[]{"e", "1"}, 4);
    ...
    void testValidity2(){
        String s2 = ("-1-----1-1-1-1--");
        TabString measureLine = new GuitarMeasureLine(s2, new String[]{"B", "1"}, 4);
    ...
    void testValidity3(){
        String s3 = ("|-----------------------------28-|");
        TabString measureLine = new GuitarMeasureLine(s3, new String[]{"D", "1"}, 4);
        Integer[] expected = {28};
        for(int i = 0; i < measureLine.noteList.size(); i++){
            GuitarNote note = (GuitarNote) measureLine.noteList.get(i);
            assertEquals(expected[i], note.getFret());
    ...
    void testValidity4(){
        String s6 = ("|----2--4--7-7----6---|");
        TabString measureLine = new GuitarMeasureLine(s6, new String[]{"G", "1"}, 4);
        assertTrue(measureLine.validate().isEmpty());
    }
    @Test
    void testInvalidity1(){
        String s4 = ("----------|----------");
        TabString measureLine = new GuitarMeasureLine(s4, new String[]{"B", "1"}, 4);
        assertFalse(measureLine.validate().isEmpty());
    }
    @Test
    void testInvalidity2(){
        String s5 = ("----2----|---4----8--|");
        TabString measureLine = new GuitarMeasureLine(s5, new String[]{"g", "1"}, 4);
        assertFalse(measureLine.validate().isEmpty());
    }
    @Test
    void testInvalidNaming(){
        ...
        for(int i = 0; i < names.length; i++){
            TabString measureLine = new GuitarMeasureLine(s6, new String[]{names[i], "1"}, 4);
            assertFalse(measureLine.validate().isEmpty());
        }
    }
//    @Test
